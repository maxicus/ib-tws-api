 ...
 */
export function request_mktData(serverVersion: number, p: any) {
  ...
  if (p.contract.secType == 'BAG' && p.contract.comboLegs.length > 0) {
    p.comboLegsCount = p.contract.comboLegs.length;
    flds.push(p.comboLegsCount);
    p.contract.comboLegs.forEach((comboLeg: any) => {
 ...
 * returns requestId
 */
export function request_tickByTickData(serverVersion: number, p: any) {
...
export function request_cancelTickByTickData(serverVersion: number, requestId: number) {
  if (serverVersion < ServerVersion.MIN_SERVER_VER_TICK_BY_TICK) {
    throw new Error('It does not support tick-by-tick data requests.');
  }
